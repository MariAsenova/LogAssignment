namespace LogTest
{
    ...
    public class AsyncLog_NotUsed : ILog
    {
        ...
        private void MainLoop()
        {
            while (!this._exit)
            {
                ...
                {
                    int f = 0; // limit the number of processed lines per iter
                    List<LogLine> _handled = new List<LogLine>(); // store the processed log lines
                    ...
                    foreach (LogLine logLine in this._lines)
                    {
                        ...
                        if (!this._exit || this._QuitWithFlush)
                        {
                            ...
                            if ((DateTime.Now - _curDate).Days != 0) // check if new day has started
                            {
                    ...
                    // if all lines have been writen - exit the main loop
